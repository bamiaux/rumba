...
fn main() {
    ...
    let mut cases: Vec<_> = corpus::DATASETS
        .iter()
        .filter(|dataset| dataset.name == "qsynth_ea")
        .flat_map(|dataset| {
    ...
    match args[1].as_str() {
        ...
        "rank" => {
            ...
            let mut samples = vec![Vec::with_capacity(5); cases.len()];
            for _ in 0..5 {
                let inputs: Vec<Expr> = cases.iter().map(|(_, e)| e.clone()).collect();
                for (e, timings) in inputs.into_iter().zip(&mut samples) {
                    let start = Instant::now();
                    let _ = black_box(simplify_mba(e, 64));
                    timings.push(start.elapsed().as_secs_f64());
                }
            }
            println!("case,nodes,variables,median_seconds");
            for ((id, e), times) in cases.iter().zip(&mut samples) {
                times.sort_by(f64::total_cmp);
                println!("{id},{},{},{:.9}", e.size(), e.get_vars().len(), times[2]);
            }
        }
        "cohort" => {
            for (id, e) in cases {
