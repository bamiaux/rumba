...
work=$(mktemp -d /tmp/rumba-qsynth.XXXXXX)
printf 'Isolated sources, binaries and measurements: %s\n' "$work"
for variant in before after; do
    mkdir "$work/$variant"
    git -C "$repo" archive HEAD | tar -x -C "$work/$variant"
    cp -a "$repo/core/src/." "$work/$variant/core/src/"
    cp "$repo/docs/performance/qsynth/probe.rs" "$work/$variant/core/examples/qsynth_probe.rs"
...
patch --directory "$work/before" -R -p1 < "$repo/docs/performance/qsynth/changes.patch"
for variant in before after; do
    CARGO_TARGET_DIR="$work/build-$variant" cargo build --locked --release \
        ...
        --example qsynth_probe --example phase3_probe --example phase3_synthetic
    sha256sum "$work/build-$variant/release/examples/qsynth_probe"
...
for variant in before after; do
    taskset -c 2 "$work/build-$variant/release/examples/qsynth_probe" rank > "$work/$variant-cases.csv"
    "$work/build-$variant/release/examples/phase3_probe" quality > "$work/$variant-quality.txt"
done
