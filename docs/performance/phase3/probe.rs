...
fn cpu_ns() -> u64 {
    fs::read_to_string("/proc/self/schedstat").unwrap()
...
fn main() {
    ...
    if args[1] == "quality" {
        ...
        for dataset in corpus::DATASETS {
            for row in corpus::rows(dataset.name, dataset.contents) {
                ...
                let Ok(actual) = simplify_mba(mba.clone(), 64) else { ng += 1; continue; };
    ...
    let mut cases: Vec<_> = corpus::DATASETS.iter().flat_map(|dataset| {
        corpus::rows(dataset.name, dataset.contents).into_iter().map(|row| {
    ...
    match args[1].as_str() {
        "cohort" => {
            for (id, e) in cases {
                if linear(&e.reduce(64)) { println!("{id}"); }
            }
        }
        "snapshot" => {
        ...
        "once" => {
            for (_, e) in cases { let _ = black_box(simplify_mba(e, 64)); }
        }
        "bench" => {
            ...
            for run in 0..6 {
                ...
                for e in inputs { let _ = black_box(simplify_mba(e, 64)); }
                let elapsed = start.elapsed().as_secs_f64();
                let cpu = (cpu_ns() - cpu_start) as f64 / 1e9;
                println!("run={run} count={} seconds={elapsed:.9} cpu={cpu:.9}", cases.len());
                if run != 0 { runs.push(elapsed); cpu_runs.push(cpu); }
            }
